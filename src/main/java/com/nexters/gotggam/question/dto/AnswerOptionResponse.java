...
public record AnswerOptionResponse(
    ...
    @Schema(description = "선택지 내용", example = "당연. 커피 수혈은 필수다.", requiredMode = Schema.RequiredMode.REQUIRED)
    String answer,

    @Schema(description = "선택 시 피드백", example = "흥, 사람이 아니라 카페인 자판기냥? 뭐, 이해는 한다.", requiredMode = Schema.RequiredMode.REQUIRED)
    String feedback
) {
